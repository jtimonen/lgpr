...
All parameters that are sampled have default priors which can be replaced by user-supplied priors. In the simplest case, the `prior` argument for the `lgp` function can be ignored, in which case default priors are used. The prior argument has to be a list with the following fields

```{r fields}
#TODO
```

For example, a custom prior for the magnitude parameter of the disease-related age component can be specified in the following way:
```{r custom}
#TODO
```

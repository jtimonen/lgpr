...
All parameters that are sampled have default priors which can be replaced by user-supplied priors. In the simplest case, the `prior` argument for the `lgp` function can be ignored, in which case default priors are used. [insert here what they are]
...
## Transforms
In the usual case, the prior for a parameter $X$ is defined to be the distribution $p_{X}(x)$, which is one of the distiributions listed above. However, in some cases we may want to define the prior so that $f(X) \sim p_{X}$, where $f$ is an invertible mapping. In this case, the log Jacobian of the transform is automatically added to the logarithm of the target density inside the Stan model. The user does not need to worry about this, and the can specify the prior transformation as explained below. The supported transformations and the corresponding log Jacobians are listed in the below table.

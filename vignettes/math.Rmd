...
motivation for the used modeling approaches, and the [tutorials](https://jtimonen.github.io/lgpr-usage/) for code examples.
...
## 4. The `likelihood` argument and observation models
...
of the `c_hat` argument of `lgp()` for exact details.

## 5. The `formula` argument and kernel functions
...
```{r formula, include = TRUE}
my_formula <- y ~ gp(age) + gp(age)*zs(id) + cat(group)
print(my_formula)
```

where `age`, `id` and `group` are columns of `data`. Each formula term defines what the corresponding kernel $k_j$ will be like, and what covariates and parameters it depends on. Each term adds one $\alpha$ parameter
...
| `cat()`     | Categorical (CAT)            | Categorical           | 
...
alone in a term but have to be multiplied by EQ, NS and VM. They are not 

...
test_that("lgpexpr validation works correctly", {
  ...
  b <- a
  b@fun <- "moi"
  expect_error(validObject(b), "<fun> must be one of")
  c <- a
  c@covariate <- ""
  expect_error(validObject(c), "covariate name cannot be empty")
})
...
test_that("lgpformula validation works correctly", {
  ...
  b <- a
  b@y_name <- "x"
  expect_error(validObject(b), "response variable cannot be also")
})
...
test_that("lgpscaling validation works correctly", {
  ...
  b <- a
  b@var_name <- ""
  expect_error(validObject(b), "name length must be at least 1")
  c <- a
  c@fun <- function(x) 3 * x
  expect_error(validObject(c), "<f_inv> is not an inverse function of <f>")
})

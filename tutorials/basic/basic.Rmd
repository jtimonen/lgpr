...
Note that ell are on normalized scale! Inference is on normalized scale!

## Assessing covariate relevances
...
Components can be selected using a threshold for proportion of total explained variance. We see that `id` and `group` are not selected.
...
```{r vis4, fig.width=7, fig.height=4}
plot_components(fit, x = x_pred, pred = p)
```


## Visualizing inferred components
TODO: maybe only out-of-sample

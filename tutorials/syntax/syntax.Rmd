...
to use the `lgp()` function which wraps both `create_model()` and `sample_model()` and has all the same arguments. The complete
...
are factors. In this notebook we use the `Orthodont` data from the `nlme` package, which consists of orthodontic growth measurements for 11 female and 16 male subjects from age 8 to 14.
```{r data1}
...
The `prior` argument to `lgp()` or `create_model()` speficies the prior

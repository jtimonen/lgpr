...
prior_stan_to_readable <- function(stan_dat) {
  ...
  if (D[4] > 0) {
    ...
    for (j in 1:D[4]) {
      if (D[4] > 1) {
        parnames <- c(
          paste("alpha_continuous[", j, "]", sep = ""),
          paste("ell_continuous[", j, "]", sep = "")
        )
      } else {
        parnames <- c(
          paste("alpha_continuous", sep = ""),
  ...
  if (D[5] > 0) {
    ...
    for (j in 1:D[5]) {
      if (D[5] > 1) {
        parnames <- c(
          paste("alpha_categAge[", j, "]", sep = ""),
          paste("ell_categAge[", j, "]", sep = "")
        )
      } else {
        parnames <- c(
          paste("alpha_categAge", sep = ""),
...
prior_statement <- function(parname, TYP, P, dist, row_change = TRUE) {
  ...
  if (TYP[2] == 1) {
    ...
    stop(
      "Invalid transform ", TYP[2], " for parameter '", parname,
...
check_warp_prior <- function(prior, L = 24, ...) {
  ...
  if (dist$type != "inv-gamma") {
    stop(
      "this function currently works only if prior of warp_steepness ",
      "is inv-gamma"
    )
  }
  ...
  plots <- ggpubr::ggarrange(out$plot, p,
    labels = "auto",
    nrow = 1, ncol = 2
  )
  return(plots)

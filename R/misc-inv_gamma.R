...
plot_invgamma <- function(alpha, beta, by = 0.01, max = 1.5, 
  ...
  if(IQR <= 0 || IQR >= 1){stop("IQR must be on interval (0,1)")}
  ...
  p1 <- ggplot2::ggplot(df, ggplot2::aes(x=t,y=y)) +
    ggplot2::xlab('Steepness') + ggplot2::ylab(ylab) +
    ggplot2::theme_bw()
  ...
  cs <- bayesplot::color_scheme_get("red")
  p1 <- p1 + ggplot2::geom_area(data=df_area,
                                mapping = ggplot2::aes(x=t,y=y),
                                fill = cs$mid)
  p1 <- p1 + ggplot2::geom_vline(xintercept = q1, color = cs$mid_highlight)
  p1 <- p1 + ggplot2::geom_vline(xintercept = q2, color = cs$mid_highlight)
  p1 <- p1 + ggplot2::geom_line()
  q1r <- round(q1, 3)
  q2r <- round(q2, 3)
  msg <- paste0('The ', IQR*100, '% IQR is [', q1r, ', ', q2r, '].\n')
  main <- paste0('Inv-Gamma(', alpha, ', ', beta ,')')
  p1 <- p1 + ggplot2::ggtitle(main,
                              subtitle = msg)
  if(return_quantiles){

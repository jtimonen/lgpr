# Part of the lgpr package for estimating model parameters
...
# START ADDITION (credit to Stijn, 
# https://discourse.mc-stan.org/t/using-expose-stan-functions-in-a-package/5982/3)
f <- file.path(MODELS_HOME, "stan_files", "chunks", "functions.stan")
file_name <- sub("\\.stan$", "", basename(f))
stan_func <- rstan::stanc(f, allow_undefined = TRUE,
                             obfuscate_model_name = FALSE)
rstan::expose_stan_functions(stan_func,
